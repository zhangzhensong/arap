...
struct event {
  ...
  VOIDD *eventptr;      /* Can be a vertex or the location of a circle event. */
...
#ifdef ANSI_DECLARATORS
VOIDD *trimalloc(int size)
#else /* not ANSI_DECLARATORS */
VOIDD *trimalloc(size)
int size;
#endif /* not ANSI_DECLARATORS */

{
  ...
  memptr = (VOIDD *) malloc((unsigned int) size);
  if (memptr == (VOIDD *) NULL) {
...
{
  pool->firstblock = (VOIDD **) NULL;
  pool->nowblock = (VOIDD **) NULL;
  pool->nextitem = (VOIDD *) NULL;
  pool->deaditemstack = (VOIDD *) NULL;
  pool->pathblock = (VOIDD **) NULL;
  pool->pathitem = (VOIDD *) NULL;
  pool->alignbytes = 0;
  pool->itembytes = 0;
...

{
  ...
  /* Find the first item in the pool.  Increment by the size of (VOIDD *). */
...

{
  ...
  /*   - sizeof(VOIDD *), so the stack of dead items can be maintained */
  /*       without unaligned accesses.                                */
  if (alignment > sizeof(VOIDD *)) {
    pool->alignbytes = alignment;
  } else {
    pool->alignbytes = sizeof(VOIDD *);
  }
  ...
  pool->firstblock = (VOIDD **)
    trimalloc(pool->itemsfirstblock * pool->itembytes + (int) sizeof(VOIDD *) +
              pool->alignbytes);
...
{
  while (pool->firstblock != (VOIDD **) NULL) {
    pool->nowblock = (VOIDD **) *(pool->firstblock);
    trifree((VOIDD *) pool->firstblock);
...
#ifdef ANSI_DECLARATORS
VOIDD *poolalloc(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
VOIDD *poolalloc(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */

{
  ...
  if (pool->deaditemstack != (VOIDD *) NULL) {
    newitem = pool->deaditemstack;               /* Take first item in list. */
    pool->deaditemstack = * (VOIDD **) pool->deaditemstack;
  } else {
    /* Check if there are any free items left in the current block. */
    if (pool->unallocateditems == 0) {
      /* Check if another block must be allocated. */
      if (*(pool->nowblock) == (VOIDD *) NULL) {
        /* Allocate a new block of items, pointed to by the previous block. */
        newblock = (VOIDD **) trimalloc(pool->itemsperblock * pool->itembytes +
                                       (int) sizeof(VOIDD *) +
                                       pool->alignbytes);
        *(pool->nowblock) = (VOIDD *) newblock;
        /* The next block pointer is NULL. */
        *newblock = (VOIDD *) NULL;
      }

      /* Move to the new block. */
      pool->nowblock = (VOIDD **) *(pool->nowblock);
      /* Find the first item in the block.    */
      /*   Increment by the size of (VOIDD *). */
      alignptr = (unsigned long) (pool->nowblock + 1);
      /* Align the item on an `alignbytes'-byte boundary. */
      pool->nextitem = (VOIDD *)
        (alignptr + (unsigned long) pool->alignbytes -
    ...
    pool->nextitem = (VOIDD *) ((char *) pool->nextitem + pool->itembytes);
...
#ifdef ANSI_DECLARATORS
void pooldealloc(struct memorypool *pool, VOIDD *dyingitem)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *((VOIDD **) dyingitem) = pool->deaditemstack;
...

{
  ...
  /* Find the first item in the block.  Increment by the size of (VOIDD *). */
...
#ifdef ANSI_DECLARATORS
VOIDD *traverse(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
VOIDD *traverse(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */

{
  ...
  if (pool->pathitemsleft == 0) {
    /* Find the next block. */
    pool->pathblock = (VOIDD **) *(pool->pathblock);
    /* Find the first item in the block.  Increment by the size of (VOIDD *). */
    alignptr = (unsigned long) (pool->pathblock + 1);
    /* Align with item on an `alignbytes'-byte boundary. */
    pool->pathitem = (VOIDD *)
  ...
  pool->pathitem = (VOIDD *) ((char *) pool->pathitem + pool->itembytes);
  pool->pathitemsleft--;
  return newitem;
}
...

{
  ...
  pooldealloc(&m->triangles, (VOIDD *) dyingtriangle);
}
...

{
  ...
  pooldealloc(&m->subsegs, (VOIDD *) dyingsubseg);
}
...

{
  ...
  pooldealloc(&m->vertices, (VOIDD *) dyingvertex);
}
...

{
  ...
  pooldealloc(&m->badsubsegs, (VOIDD *) dyingseg);
}
...

{
  ...
  trifree((VOIDD *) m->dummytribase);
  if (b->usesegments) {
    pooldeinit(&m->subsegs);
    trifree((VOIDD *) m->dummysubbase);
  }
...

{
  ...
  trifree((VOIDD *) m->infvertex1);  /* Deallocate the bounding box vertices. */
  trifree((VOIDD *) m->infvertex2);
  trifree((VOIDD *) m->infvertex3);
...

{
  ...
  *events = (struct event *) trimalloc(maxevents * (int) sizeof(struct event));
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    (*events)[i].eventptr = (VOIDD *) thisvertex;
  ...
  *freeevents = (struct event *) NULL;
  for (i = maxevents - 1; i >= m->invertices; i--) {
    (*events)[i].eventptr = (VOIDD *) *freeevents;
    *freeevents = *events + i;
  }
}
...

{
  ...
  if (eventvertex != (vertex) NULL) {
    ...
    deadevent->eventptr = (VOIDD *) *freeevents;
...

{
  ...
  if (checkvertex == splaytree->keydest) {
    ...
    pooldealloc(&m->splaynodes, (VOIDD *) splaytree);
...

{
  ...
  eventheap[0]->eventptr = (VOIDD *) freeevents;
...
*/
    ...
    nextevent->eventptr = (VOIDD *) freeevents;
    ...
    if (check4events) {
      ...
      if (lefttest > 0.0) {
        ...
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                   lefttest);
        newevent->eventptr = (VOIDD *) encode(lefttri);
      ...
      if (righttest > 0.0) {
        ...
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                   righttest);
        newevent->eventptr = (VOIDD *) encode(farrighttri);
        eventheapinsert(eventheap, heapsize, newevent);
        heapsize++;
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
    ...
    while ((m->badtriangles.items > 0) && (m->steinerleft != 0)) {
      ...
      if (m->badsubsegs.items > 0) {
        ...
        pooldealloc(&m->badtriangles, (VOIDD *) badtri);
      }
    }
  }
...

{
  ...
  m->vertices.deaditemstack = (VOIDD *) NULL;
...
#ifndef CDT_ONLY
  if (m.regions > 0) {
    trifree((VOIDD *) regionarray);
  }
#endif /* not CDT_ONLY */
  if (m.holes > 0) {
    trifree((VOIDD *) holearray);
  }
  if (b.geomview) {
